...
namespace PFC.Demo.ClientApp.Services
{
    public static class CuentaBancariaService
    {
        ...
        public static async Task<ResultModel<CuentaBancariaModel>> GetCuentaBancariaById(int id)
        {
            ...
            {
                if(response.GetResult<ResultModel<CuentaBancariaModel>>(out var result))
                {
                    return result;
                } else if(response.GetResult<ResultModel>(out var model))
                {
                    return new ResultModel<CuentaBancariaModel>(null, model.Message, model.Success);
                }
            }

            if (response.GetResult<HttpError>(out var error))
            {
                return new ResultModel<CuentaBancariaModel>(null, error.Message, false);
            }

            return new ResultModel<CuentaBancariaModel>(null, "Error desconocido", false);
        }
